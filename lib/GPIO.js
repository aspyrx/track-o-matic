...
class GPIO {
    ...
    constructor(pin, direction) {
        const pinPath = path.join(GPIO.BASE_PATH, `gpio${pin}`);
        ...
        const paths = {
            /** Pin direction. */
            direction: path.join(pinPath, 'direction'),
            /** Pin value. */
...
GPIO.VALUES = {
    /** Low (0) */
    LOW: Buffer.from('0', 'ascii'),
    /** High (1) */
    HIGH: Buffer.from('1', 'ascii')
};
