...
class Stepper extends EventEmitter {
     ...
     * the motor, specified as `[A, B, !A, !B]`. If a
     * [`MockStepper`]{@link module:MockStepper} is specified, it is used
     * instead.
     */
    ...
    /**
     * Calibrates the stepper's current position.
     *
     * @param {number} step - The current step value.
     */
    calibrate(step) {
        this._currentStep = Math.round(step);
    }
     ...
     */
    stepMotor() {
        const phase = this._currentStep % NUM_PINS;
        const pins = STEP_PINS[phase];
    ...
    step(steps, duration) {
        ...
        const total = Math.round(Math.abs(steps));
    ...
    /**
     * Closes the stepper's associated GPIO pins.
     */
    close() {
        if (!(this.gpios instanceof Array)) {
            return;
        }
