...
/**
 * How frequently the bus times are checked.
 *
 * @readonly
 * @type {number}
 */
const BUSTIME_INTERVAL = 60 * 1000;
...
function setupSteppers() {
    ...
    steppers[0] = new Stepper(2034, [2, 3, 4, 14]);
    steppers[1] = new Stepper(2034, [26, 21, 20, 16]);
    steppers[2] = new Stepper(2034, [15, 17, 18, 27]);
    steppers[3] = new Stepper(2034, [19, 13, 12, 6]);
    ...
    async function updateBusTimes() {
        try {
            ...
            await Promise.all(rows.map((row, i) => {
                ...
                let rt;
                if (prd) {
                    console.log(`bustime: ${prd.rt}, ${prd.prdctdn}`);
